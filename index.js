...
const canvasAssets = {  
    ...
    matchingTemplate: "https://example.com/custom_profile.png",   

    profileTemplate: "https://example.com/profile_2.jpg"        
...
أوامر الماتشينق والأفتار if (commandName === 'matching') { await interaction.reply({ content: '⏳ جاري التصميم...', ephemeral: true }); const template = await loadImage(options.getAttachment('template').url); const canvas = createCanvas(1200, 700); const ctx = canvas.getContext('2d'); ctx.drawImage(template, 0, 0, 1200, 700); constdrawCircularImage = async (url, x, y, size) => { const img = await loadImage(url); ctx.save(); ctx.beginPath(); ctx.arc(x + size / 2, y + size / 2, size / 2, 0, Math.PI * 2); ctx.closePath(); ctx.clip(); ctx.drawImage(img, x, y, size, size); ctx.restore(); }; await drawCircularImage(options.getAttachment('avatar_1').url, 150, 400, 250); awaitdrawCircularImage(options.getAttachment('avatar_2').url, 800, 400, 250); const targetChannel = awaitclient.channels.fetch(config.matchingRoomId); await targetChannel.send({ files: [{ attachment: canvas.toBuffer('image/png'), name: 'matching.png' }] }); await interaction.editReply({ content: '✅ تم إرسال الماتشينق!' }); } if (commandName === 'افتار') { await interaction.reply({ content: '⏳ جاري تصميم البروفايل...', ephemeral: true }); consttemplate = await loadImage(options.getAttachment('template').url); const canvas = createCanvas(1200, 700); const ctx = canvas.getContext('2d'); ctx.drawImage(template, 0, 0, 1200, 700); const avatar = awaitloadImage(interaction.user.displayAvatarURL({ extension: 'png' })); ctx.save(); ctx.beginPath(); ctx.arc(200, 500, 125, 0, Math.PI * 2); ctx.closePath(); ctx.clip(); ctx.drawImage(avatar, 75, 375, 250, 250); ctx.restore(); const targetChannel = await client.channels.fetch("1501583456872829068"); await targetChannel.send({ files: [{ attachment: canvas.toBuffer('image/png'), name: 'profile.png' }] }); await interaction.editReply({ content: '✅ تم إرسال الأفتار للروم المطلوب!' }); } } // 2. التعامل مع الأزرار (Buttons) if (interaction.isButton()) { const { customId, guildId, user } = interaction; if(customId === 'join_roulette') { /* ... */ } if (customId === 'start_roulette') { /* ... */ } if (customId === 'join_mafia') { /* ... */ } if (customId === 'start_mafia') { /* ... */ } } });
